...
x_ini = [140; 40; 0]; %The initial state of the sensor of the robot,
...
[situation,check_point]=initial_situation(c_ini(1),c_ini(2));%Check the initial position
...
%//////////////////////// The Pre-Parking Process /////////////////////////
    if situation==0 %When the car is close to the walls or obstacles
  ...
  [x1, y1] = new_state_evo_to_sensor(x_ini, u_s0{1});
  [x2, y2] = new_state_evo_to_sensor(x1, u_s0{2});
  [x3, y3] = new_state_evo_to_sensor(x2, u_s0{3});
  [x4, y4] = new_state_evo_to_sensor(x3, u_s0{4});
  [x5, y5] = new_state_evo_to_sensor(x4, u_s0{5});
  ...
  x_joint=x4;%Store the last state of the pre-parking process into the joint state
        ...
        %Get the state of each milestone point on the path
        [x1, y1] = new_state_evo_to_sensor(x_ini, u_s1{1});
        [x2, y2] = new_state_evo_to_sensor(x1, u_s1{2});
        [x3, y3] = new_state_evo_to_sensor(x2, u_s1{3});
        
        %Displaying the pre-parking simulation for situation1
        pause on;
        new_plot_car(x_ini);
        pause(2);
        trajectory_display(x_ini,x1); 
...
%//////////////////////// The Post-Parking Process ////////////////////////
       ...
       %Get the state of each milestone point on the path
       [x1, y1] = new_state_evo_to_sensor(x_start, u{1});
       [x2, y2] = new_state_evo_to_sensor(x1, u{2});
       [x3, y3] = new_state_evo_to_sensor(x2, u{3});
